...
import { OrderModule, DispatchModule, BillModule, OrderItemModule, OrderItemDetailModule } from './dispatch.module'
...
export class DispatchComponent implements OnInit {
  ...
  model: any = 'QWERTY'
  order: OrderModule
  orderdetail: OrderItemDetailModule
  Disp: DispatchModule
  ...
  getorderPrd(orderId) {
    ...
    if (this.OrdId != 0) {
      this.Auth.getorderPrd(this.loginfo.companyId, this.OrdId).subscribe(data => {
        ...
        console.log('hhhhhhhhhhrtughggg', this.ordPrdDetails, this.order)
        this.ordPrdDetails.orderProd.forEach(element => {
          console.log(this.ordPrdDetails.orderProd)
          element.Action = 'Chk'
          ...
          this.array.push({
            ...
            OrdItemDetailId: element['orderitemdetail'],
        ...
        this.order.Items = [...this.toOrdItems(this.ordPrdDetails.orderProd)]
  ...
  StoreByIdInternal(storeId) {
    this.Auth.getstoreIdInternal(this.loginfo.companyId, storeId).subscribe(data => {
      ...
      this.ordPrdDetails.order.forEach(order => {
  ...
  dispatchstatus = 2
  toOrdItems(rawItems: any[]) {
    let items: Array<OrderItemModule> = []
    rawItems.forEach(item => {
      let oItem: OrderItemModule = new OrderItemModule(item, item.orderId, this.loginfo.storeId)
      oItem.Id = item.orderItemId
      oItem.OrderItemId = oItem.Id
      oItem.Action = "Chk"
      oItem.ActualProdId = item.productId
      oItem.Amount = item.amount
      oItem.BarcodeId = item.barcodeId
      oItem.BatchNum = null
      oItem.BillDate = moment().format('YYYY-MM-DD HH:MM A')
      oItem.BillDateTime = moment().format('YYYY-MM-DD HH:MM A')
      oItem.BillId = 0
      oItem.CancelledQuantity = null
      oItem.CompanyId = item.companyId
      oItem.ContainerCount = item.containerCount
      oItem.ContainerId = item.containerId
      oItem.ContainerName = null
      oItem.CreatedDate = item.createdDate
      oItem.DiscPercent = null
      oItem.DispatchProductId = item.productId
      oItem.DispatchQty = item.orderQuantity
      oItem.Dispatchprd = item.name
      oItem.GrossQty = item.orderQuantity
      oItem.OrdItemType = item.ordItemType
      oItem.OrderId = item.orderId
      oItem.OrderQuantity = item.orderQuantity
      oItem.DispatchedQuantity = oItem.OrderQuantity
      oItem.ProductName = item.name
      oItem.Price = item.price
      oItem.ProductId = item.productId
      oItem.StorageStoreId = null
      oItem.Tax1 = item.tax1
      oItem.Tax2 = item.tax2
      oItem.Tax3 = item.tax3
  ...
  OrdDispatch() {
    console.log(this.order.DispatchStatus)
    this.order.BillDate = moment().format('YYYY-MM-DD HH:MM A')
    this.order.CreatedDate = moment().format('YYYY-MM-DD HH:MM A')
    this.order.BillDateTime = moment().format('YYYY-MM-DD HH:MM A')
    this.order.OrderedDate = moment().format('YYYY-MM-DD HH:MM A')
    ...
    this.order.Items.forEach(item => {
      item.DispatchedQuantity = item.OrderQuantity

    })
    var finalarray = [...this.order.Items]
    ...
    this.Disp.BillAmount = this.order.BillAmount
    this.Disp.Quantity = this.order.Items.map(x => x.OrderQuantity).reduce((a, b) => a + b, 0)
    this.Disp.TotalAmount = this.order.Subtotal
    this.Disp.TaxAmount = this.order.TaxAmount
    this.Disp.DispatchStatus = 2
   

    console.log('finalarray', finalarray)
    console.log('final5555array', this.Disp)
    console.log(JSON.stringify(this.Disp.Items))
    // return
