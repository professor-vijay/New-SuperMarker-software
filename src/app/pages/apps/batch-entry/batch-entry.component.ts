...
export class BatchEntryComponent implements OnInit {
  ...
  pushintobatch() {
    ...
    if (this.validation()) {
      ...
      if (this.batches.some(x => x.productId == this.batchentry['productId'])) {
        this.batches.filter(x => x.productId == this.batchentry['productId'],)[0].quantity += this.batchentry.quantity
        console.log(this.batches)
        console.log(this.batchentry)
      } else {
        this.batches.push(this.batchentry);
      }
      this.batchentry = { barCode: null, code: '', barcodeId: null, quantity: null, price: null, expiarydate: "", companyid: this.loginfo.companyId, storeid: this.loginfo.storeId, productId: 0, product: null, batchno: 0, entrydatetime: "" }
  ...
  validation() {
    ...
    if (this.batchentry.quantity > this.batchentry.quantity) isvalid = false
